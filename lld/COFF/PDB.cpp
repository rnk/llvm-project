...
static void scopeStackClose(SmallVectorImpl<uint32_t> &stack,
  ...
  ScopeRecord *scopeRec = reinterpret_cast<ScopeRecord *>(
      &(storage)[offOpen + sizeof(RecordPrefix)]);
...
static void fixRecordAlignment(MutableArrayRef<uint8_t> recordBytes,
  ...
  if (oldSize == alignedSize)
    return;
  reinterpret_cast<RecordPrefix *>(recordBytes.data())->RecordLen = alignedSize - 2;
...
void DebugSHandler::finish() {
  ...
  if (moduleStreamSize > 4)
    file.moduleDBI->addUnmergedSymbols(&file, moduleStreamSize - 4);
