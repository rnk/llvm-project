  ...
  if (kind == SymbolKind::S_GPROC32_ID || kind == SymbolKind::S_LPROC32_ID) {
    ...
    if (!ti->isSimple() && !ti->isNoneType()) {
      if (config->debugGHashes) {
        // TODO: We should be able to implement this through the ghash map, or
        // by building a map from LF_[MEM]FUNC_ID to source FunctionType.
        *ti = TypeIndex(0);
      } else {
...
void PDBLinker::mergeSymbolRecords(ObjFile *file, CVIndexMap &indexMap,
  ...
  cantFail(forEachCodeViewRecord<CVSymbol>(
      symsBuffer, [&](CVSymbol sym) -> llvm::Error {
        ...
        translateIdSymbols(recordBytes, tMerger);
        sym = CVSymbol(recordBytes);
