...
void PDBLinker::mergeSymbolRecords(ObjFile *file, CVIndexMap &indexMap,
  ...
  cantFail(forEachCodeViewRecord<CVSymbol>(
      symsBuffer, [&](CVSymbol sym) -> llvm::Error {
        ...
        if (!file->debugTypesObj->remapTypesInSymbolRecord(recordBytes,
                                                           indexMap)) {
...
void DebugSHandler::mergeInlineeLines(
  ...
  for (const InlineeSourceLine &line : inlineeLines) {
    ...
    if (!file.debugTypesObj->remapTypeIndex(inlinee, TiRefKind::IndexRef,
                                            *indexMap)) {
...
// FIXME: Make indexMap const for thread safety reasons.
...
void PDBLinker::addDebug(TpiSource *source) {
  ...
  addDebugSymbols(source->file, &source->indexMap);
}
