...
1. `sizeof(SourceLocation)` must be extremely small, as these are embedded
   into many AST nodes and are passed around often. Currently, it is 32 bits.
2. `SourceLocation` must be a simple value object that can be efficiently
   copied.
3. We should be able to represent a source location for any byte of any input
   file. This includes in the middle of tokens, in whitespace, in trigraphs,
   etc.
4. A `SourceLocation` must encode the current `#include` stack that was
   ...
   for a diagnostic.
5. `SourceLocation` must be able to describe macro expansions, capturing both
...
- **Flags** --- There are currently four flags tracked by the
  ...
  1. **StartOfLine** --- This was the first token that occurred on its input
     source line.
  2. **LeadingSpace** --- There was a space character either immediately before
  ...
  3. **DisableExpand** --- This flag is used internally to the preprocessor to
     represent identifier tokens which have macro expansion disabled. This
     prevents them from being considered as candidates for macro expansion ever
     in the future.
  4. **NeedsCleaning** --- This flag is set if the original spelling for the
...
1. **tok::annot_typename**: This annotation token represents a resolved
...
2. **tok::annot_cxxscope**: This annotation token represents a C++ scope
...
3. **tok::annot_template_id**: This annotation token represents a C++
...
are especially useful for performing [flow- or path-sensitive](https://en.wikipedia.org/wiki/Data_flow_analysis#Sensitivities) program
...
As per the C++ standard, constraints are [normalized](https://eel.is/c++draft/temp.constr.normal)
...
[include/clang/Basic/Attr.td](https://github.com/llvm/llvm-project/blob/main/clang/include/clang/Basic/Attr.td).
...
automatically determine the diagnostic parameter in [utils/TableGen/ClangAttrEmitter.cpp](https://github.com/llvm/llvm-project/blob/main/clang/utils/TableGen/ClangAttrEmitter.cpp)
...
stand-alone definition in [include/clang/Basic/AttrDocs.td](https://github.com/llvm/llvm-project/blob/main/clang/include/clang/Basic/AttrDocs.td)
...
be created, but it requires modifying [utils/TableGen/ClangAttrEmitter.cpp](https://github.com/llvm/llvm-project/blob/main/clang/utils/TableGen/ClangAttrEmitter.cpp)
...
and the parsing code in [Parser::ParseGNUAttributeArgs()](https://github.com/llvm/llvm-project/blob/main/clang/lib/Parse/ParseDecl.cpp)
can be updated for the special case. Note that this only applies to arguments
with a GNU spelling -- attributes with a \_\_declspec spelling currently ignore
...
All semantic processing of declaration attributes happens in [lib/Sema/SemaDeclAttr.cpp](https://github.com/llvm/llvm-project/blob/main/clang/lib/Sema/SemaDeclAttr.cpp),
...
[include/clang/Basic/DiagnosticGroups.td](https://github.com/llvm/llvm-project/blob/main/clang/include/clang/Basic/DiagnosticGroups.td)
...
directly in [DiagnosticSemaKinds.td](https://github.com/llvm/llvm-project/blob/main/clang/include/clang/Basic/DiagnosticSemaKinds.td)
...
1. Introduce parsing actions into the parser. Recursive-descent parsing is
...
2. Introduce semantic analysis actions into `Sema`. Semantic analysis should
...
4. Teach semantic analysis to build your AST node. At this point, you can wire
...
5. Teach code generation to create IR to your AST node. This step is the first
...
7. There are some "extras" that make other features work better. It's worth
...
language support page for the corresponding feature ([C++](https://clang.llvm.org/cxx_status.html), [C](https://clang.llvm.org/c_status.html)) if applicable. This page can give
